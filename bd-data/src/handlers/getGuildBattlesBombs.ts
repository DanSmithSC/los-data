...
const { TABLE_NAME, GUILD_TAG, GUILD_ID, USER_ID, INSTALL_ID, CONFIG_BUCKET, CONFIG_FILE, GOOGLE_SPREADSHEET } = process.env
...
const getLeaderboardCurrentTierSet = async (sessionId, configValues) => {
  ...
  try {
    const response = await axios.post(`${snowprintURL}${USER_ID}/sessionId/${sessionId}`, getGuildSeasonLeaderboard2, {
      headers : {
    ...
    const guildState = response.data.eventResult.eventResponseData.leaderboard.local.find( guild => {return  guild.guildTag === GUILD_TAG})
    // console.log(guildState)
    const currentLeaderboardSeason = response.data.eventResult.eventResponseData.leaderboard.season+1
    
    const circuit = 
      !guildState.loopIndex
        ? guildState.tierReached ? Math.floor(guildState.tierReached / 5 + 1 ) : 1
...
const getBossData = async (
  ...
  const condensedBattlesBossOrder = guildBossData.filter( event => 
    event.type === 'GuildBossEncounterCompleted' &&
    event.data.damageType === 'Battle' &&
    event.seq > lastRecordedEvent 
  )
  
  condensedBattlesBossOrder.length ? condensedBattlesBossOrder.map( battle => {
    return battle.data.levelId ? parseInt(battle.data.levelId.slice(-8,-3).replace(/_+/g, ' ')) : null
  }) : []

  const minOfCondensedBattlesBossOrder = condensedBattlesBossOrder.length ? Math.min(...condensedBattlesBossOrder) : []

  const indexOfMinCondensedBattlesBossOrder = condensedBattlesBossOrder.indexOf(minOfCondensedBattlesBossOrder)
  
  const newBossBattles = guildBossData.filter( event => 
    event.type === 'GuildBossEncounterCompleted' &&
    event.data.damageType === 'Battle' &&
    event.seq > lastRecordedEvent 
  )
  ...
  condensedBombsBossOrder.length ? condensedBombsBossOrder.map( bomb => {
    return parseInt(bomb.data.levelId.slice(-8,-3).replace(/_+/g, ' '))
  }) : []

  // console.log(condensedBombsBossOrder)

  const minOfCondensedBombsBossOrder = condensedBombsBossOrder.length ? Math.min(...condensedBombsBossOrder) : []
  // console.log(minOfCondensedBattlesBossOrder)

  const indexOfMinCondensedBombsBossOrder = condensedBombsBossOrder.indexOf(minOfCondensedBombsBossOrder)
  // console.log(indexOfMinCondensedBombsBossOrder)  

  const newBossBombs = guildBossData.filter( event => 
    event.type === 'GuildBossEncounterCompleted' &&
    event.data.damageType === 'Bomb' &&
    event.seq > lastRecordedEvent
  )
  
  const guildBossBombs = newBossBombs.length ? newBossBombs.map( (bomb, index) => {
    ...
    if( lastRecordedDungeonRound < circuit){
      index < indexOfMinCondensedBombsBossOrder || bomb.data.levelId.slice(-6,-3) === '004' ? round = lastRecordedDungeonRound : round = circuit          
    }
    ...
    return {
      ...
      date: new Date(bomb.timestamp).toLocaleDateString('en-US', {timeZone: "America/Chicago"}),
      season: currentLeaderboardSeason,
      circuit: round,
  ...
  const battlesMappedForSheets = guildBossBattles.length ? guildBossBattles.map( battle => {
    return [
      battle.eventId,
      battle.timestamp,
      battle.player,
  ...
  const bombsMappedForSheets = guildBossBombs.length ? guildBossBombs.map( bomb => {
    return [
      bomb.eventId,
      bomb.timestamp,
      bomb.player,
  ...
  return {guildBossData, guildBossBattles, guildBossBombs, battlesMappedForSheets, bombsMappedForSheets, lastEventSeq}
...
module.exports.getGuildBattlesBombs = async (event:any) => {
  ...
  const auth = new sheets.auth.GoogleAuth({
    keyFilename: path.join(__dirname, '../../data/dragonbot-1567112394071-e02f73d3144e.json'),
  ...
  const { currentLeaderboardSeason, circuit } = await getLeaderboardCurrentTierSet(sessionId, configValues)
  // console.log(guildState, circuit)

  const {guildBossBattles, guildBossBombs, battlesMappedForSheets, bombsMappedForSheets, lastEventSeq} = await getBossData(sessionId, configValues, lastRecordedEvent, round, currentLeaderboardSeason, circuit)
  ...
  try {
    if(guildBossBattles.length) {
      ...
      try {
        console.log(`Calling Write to Sheet with New BATTLES: ${JSON.stringify(guildBossBattles, null,2)}`)
    ...
    if(guildBossBombs.length){
      ...
      try {
        console.log(`Calling Write to Sheet with New BATTLES: ${JSON.stringify(guildBossBombs, null,2)}`)
  ...
  try {
    ...
    console.log(`Round Recorded: ${circuit} | Last EventRecorded Updated to: ${lastEventSeq} `)
  } catch(err){
    console.error(err)
  }
