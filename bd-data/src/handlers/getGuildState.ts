...
module.exports.getGuildState = async (event:any) => {
  ...
  console.log(lastRecordedEvents)
  ...
  if( lastRecordedEvents.eventId < guildEventsState.lastGuildBossEventId ){
    ...
    await invokeLambda(payload, 'bd-data-dev-getGuildBattlesBombs')
    console.log(`New Events To Process`)
    console.log(payload)
    return {
      message: "Calling Next Function: bd-data-dev-getGuildBattlesBombs",
      payload: payload
    }
  }
  
  return console.log(`Guild Events Checked - No New Events to Pull - Last EventID ${guildEventsState.lastGuildBossEventId}`)
