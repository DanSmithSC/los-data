...
const getSession = async (userInfo) => {
  try {
    ...
    const sessionId =
      response.data.eventResult.eventResponseData.userData.sessionId;
    return sessionId;
  } catch (err) {
    console.error(err);
  }
...
const getDungeonSeasonData = async (sessionId, configValues) => {
  ...
  try {
    ...
    // // console.log(
    //   response.data.eventResult.eventResponseData.player.guild.guildBossGameMode
    // );

    // console.log(response.data.eventResult.eventResponseData.player.guild.guildBossGameMode.currentSet.encounters)
    console.log(
      `Last Guild State Event IDs:`,
      response.data.eventResult.eventResponseData.player.guildEventsState
    );
    const guildEventsState =
      response.data.eventResult.eventResponseData.player.guildEventsState;
...
const getLastRecordedState = async () => {
  ...
  try {
    ...
    if (data) {
      ...
      const eventId = data.Responses[TABLE_NAME].find(
        (item) => item.eventId
      ).eventId;
...
module.exports.getGuildState = async (event: any) => {
  ...
  const sessionId = await getSession(userInfo).then((value) => {
    return value;
  });
  console.log(`SessionId | ${sessionId}`);

  const { guildEventsState, season } = await getDungeonSeasonData(
    sessionId,
    configValues
  );
  ...
  if (lastRecordedEvents.eventId < guildEventsState.lastGuildBossEventId) {
    ...
    await invokeLambda(payload, 'vg-data-dev-getGuildBattlesBombs');
    return {
      message: 'Calling Next Function: vg-data-dev-getGuildBattlesBombs',
      payload: payload,
    };
  }
